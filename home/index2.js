  ...
  function addItemToDatabase(tableId, itemId, quantity) {
    ...
    .then(data => {
        ...
        if (data.exists) {
            ...
            // If the item doesn't exist, add it with the given initial quantity
...
//     // Fetch all items in the current table
//     const itemsTableBody = document.getElementById('itemsTableBody');
//     const rows = itemsTableBody.querySelectorAll('tr');

//     try {
//         // Check if any item in the table matches the barcode
//         for (const row of rows) {
//             const itemBarcode = row.cells[2].textContent.trim();
//             if (itemBarcode === barcode) {
//                 const itemName = row.cells[1].textContent.trim();
//                 const itemPrice = parseFloat(row.cells[4].textContent.trim());

//                 // Perform the addItemToDatabase function with the matched item details
//                 await addItemToDatabase(currentOpenTableId, itemId, 1); // Assuming itemId is defined elsewhere
//                 await addNewItemToTable(currentOpenTableId, itemId, 1); // Assuming itemId is defined elsewhere
//                 await updateItemQuantity(currentOpenTableId, itemId, 1); // Assuming itemId is defined elsewhere

//                 // Add the matched item to the table
//                 addItemToTable({
//                     id: itemId, // Replace with actual item ID
//                     name: itemName,
//                     barcode: itemBarcode,
//                     price: itemPrice
//                 });
...
//         // If no match found in the table items, search the database by barcode
//         const response = await fetch(`./searchByBarcode.php?barcode=${barcode}`);
//         if (!response.ok) {
//             throw new Error('Network response was not ok');
//         }
...
//         if (item) {
//             // Perform the addItemToDatabase function with the matched item details
//             await addItemToDatabase(currentOpenTableId, item.id, 1); // Assuming item.id is the database ID of the item
//             await addNewItemToTable(currentOpenTableId, item.id, 1); // Assuming item.id is the database ID of the item
//             await updateItemQuantity(currentOpenTableId, item.id, 1); // Assuming item.id is the database ID of the item
...
//             document.getElementById('searchResults').innerHTML = '';
//         } else {
//             // No item found with the entered barcode
...
function handleBarcode(event) {
    ...
    if (/^[0-9a-zA-Z]+$/.test(searchInput)) {
        // If input is a barcode-like string, automatically search by barcode
        fetch(`./search.php?barcode=${searchInput}`)
            ...
            .then(data => {
                if (data && data.length > 0) {
                    console.log(data);
                    // Assuming only one item should match, print the item details
                    console.log('Item found:', data[0]);
                    // Example: Add item to table or display details in an element
...
function addItemToTable(item) {
    ...
    let quantity = 1;
    addItemToDatabase(currentOpenTableId, item.id, quantity);
    //addItemToDatabase(currentOpenTableId, item.id, quantity);
    rows.forEach(row => {
        if (row.cells[2].textContent === item.barcode || row.cells[1].textContent === item.name) {
            ...
            quantity = parseInt(quantityCell.value, 10);
            quantityCell.value = quantity + 1;

            const price = parseFloat(item.price);
            const totalCell = row.cells[5];
            const newTotal = ((quantity + 1) * price).toFixed(2);
...
async function addItemToTable(item) {
    ...
    if (!itemExists) {
        ...
        // addItemToDatabase(currentOpenTableId, item.id, 1);
        await addItemToDatabase(currentOpenTableId, item.id, 1); // Assuming itemId is defined elsewhere
                        await addNewItemToTable(currentOpenTableId, item.id, 1); // Assuming itemId is defined elsewhere
                        await updateItemQuantity(currentOpenTableId, item.id, 1); // Assuming itemId is defined elsewhere
    }

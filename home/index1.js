    ...
    try {
        ...
        for (const row of rows) {
            ...
            if (itemBarcode === barcode) {
                ...
                // Add the matched item to the table display
                addItemToTable({
                ...
                // Return here if item found in the table
                return;
            }
        }
        ...
        if (item) {
            // Log the item details found in the database
            console.log('Item found by barcode:', item);
        } else {
        ...
        fetch(`./search.php?barcode=${searchInput}`)
            ...
            .then(data => {
                if (data && data.length > 0) {
                     addItemToTable(data[0]); // Assuming only one item should match
                    document.getElementById('searchResults').style.display = 'none';
                } else {

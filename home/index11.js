...
//    currentDay();
    ...
    async function displayTables(tables, tablesWithInvoices) {
        ...
        try {
            ...
            // Define filtered tables and display function
            let filteredTables;
            const tables1to23Button = document.getElementById('tables1to23');
            const tables24to46Button = document.getElementById('tables24to46');
    
            // Event listener for tables 1-23 button
            tables1to23Button.addEventListener('click', () => {
                filteredTables = tables.filter(table => table.tableId >= 1 && table.tableId <= 23);
                display(filteredTables, '1to23',mytables);
            });
    
            // Event listener for tables 24-46 button
            tables24to46Button.addEventListener('click', () => {
                filteredTables = tables.filter(table => table.tableId >= 24 && table.tableId <= 46);
                display(filteredTables, '24to46',mytables);
            });
    
            // Initial display (default to tables 1-23)
            filteredTables = tables.filter(table => table.tableId >= 1 && table.tableId <= 23);
            display(filteredTables, '1to23');
    
            async function display(tablesToDisplay, activeButton, mytables) {
                console.log(tablesToDisplay, '---------------------');
                
                // Clear previous displayed tables
                tablesContainer.innerHTML = '';
            
                tablesToDisplay.forEach((table, index) => {
                    const tableDiv = document.createElement('div');
                    tableDiv.classList.add('table');
                    tableDiv.dataset.tableId = table.tableId;
            
                    if (index === tablesToDisplay.length - 1) {
                        tableDiv.textContent = 'بيع بدون طاولة'; // Special table text
                    ...
                    // Check if table is reserved and update accordingly
                    if (table.status === 'reserved') {
                        tableDiv.style.width = '18rem';
                        tableDiv.textContent = `${table.tableId} - ${table.customerName}`;
                    }
            
                    // Disable table if cannot sell
                    if (!canSell) {
                ...
                // Toggle visibility based on active button
                if (activeButton === '1to23') {
                    tables1to23Button.classList.add('active');
                    tables24to46Button.classList.remove('active');
                } else if (activeButton === '24to46') {
                    tables24to46Button.classList.add('active');
                    tables1to23Button.classList.remove('active');
                }
            }
        ...
        .then(async (data) => {
            if (data.success) {
                ...
                tables.forEach(table => {
                    console.log(table.dataset.tableId); // Log the data-table-id attribute
                
                    // Check if the current table's dataset table id matches the current open table id
                    if (table.dataset.tableId === currentOpenTableId) {
                        console.log('here')
                        table.style.width = '16rem';
                        // Update dataset attribute with customer name (example)
                        table.textContent = table.dataset.tableId + ' - ' + data.customerName;
                    }
                });
                ////////////////////////////////////////////////////////
            } else {
...
async function fetchTableStatuses() {
    fetch('fetch_table_statuses.php')
        ...
        .then(data => {
            console.log(data)
            const tablesDivs = document.querySelectorAll('div.table');

          
            data.forEach(table => {
                var tId = table.tableId;
                tablesDivs.forEach(tableDiv => {
                    // console.log(tableDiv.dataset.tableId); // Log the data-table-id attribute
                
                    // Check if the current table's dataset table id matches the current open table id
                    if (tableDiv.dataset.tableId === tId) {
                        // console.log('here')
                        tableDiv.style.width = '18rem';
                        // Update dataset attribute with customer name (example)
                        tableDiv.textContent = tId + ' - ' + table.customerName;
                    }
                });
                ...
                var tableElement = document.querySelector('.table[data-table-id="' + tId + '"]');

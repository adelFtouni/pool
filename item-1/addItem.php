...
if (!isset($data['itemBarcode'], $data['itemName'], $data['itemPrice'], $data['categoryId'])) {
...
if ($count > 0) {
    ...
    // Insert new item
    $insertStmt = $conn->prepare("INSERT INTO `item` (`barcode`, `name`, `price`, `display_home`, `category_id`) VALUES (?, ?, ?, ?, ?)");
    $insertStmt->bind_param("ssdis", $itemBarcode, $itemName, $itemPrice, $displayHome, $categoryId);

...
function editItem(row, id, barcode, name, price, displayHome, category) {
    ...
    actionCell.querySelector('.cancel-button').addEventListener('click', () => cancelEdit(row, barcode, name, price, displayHome, category));
}
...
function saveItem(row, id) {
    ...
    if (barcode && name && price && !isNaN(price) && categoryID) {
        const data = {
            id: id,
            itemBarcode: barcode,
            itemName: name,
    ...
    fetch('get_items.php')
        ...
        .then(items => {
            ...
            items.forEach(item => {
                ...
                newRow.querySelector('.edit-button').addEventListener('click', () => editItem(newRow, item.id, item.barcode, item.name, item.price, item.display_home, item.category_name));

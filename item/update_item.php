...
if (!isset($data['id'], $data['itemBarcode'], $data['itemName'], $data['itemPrice'], $data['categoryID'], $data['itemQuantity'])) {
...
// Update the item
$stmt = $conn->prepare("UPDATE `item` SET `barcode` = ?, `name` = ?, `price` = ?, `display_home` = ?, `category_id` = ?, `quantity` = ? WHERE `id` = ?");
$stmt->bind_param("ssdiiii", $itemBarcode, $itemName, $itemPrice, $displayHome, $categoryID, $itemQuantity, $id);
